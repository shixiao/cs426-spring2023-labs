...
func MakeVideoServiceServer(options VideoServiceOptions) *VideoServiceServer {
	...
	log.Printf(
		"Starting VideoService with failure injection config: [sleepNs: %d, failureRate: %d, responseOmissionRate: %d]",
		fiConfig.SleepNs,
...
func (db *VideoServiceServer) GetVideo(
	...
	if len(deduplicateIds(videoIds)) != len(videoIds) {
		return nil, status.Error(
			codes.InvalidArgument,
			"VideoService: duplicate IDs found in video_ids",
		)
	}
...
func (db *VideoServiceServer) SetInjectionConfig(
	...
	log.Printf(
		"VideoService failure injection config set to: [sleepNs: %d, failureRate: %d, responseOmissionRate: %d]",
		req.Config.SleepNs,
